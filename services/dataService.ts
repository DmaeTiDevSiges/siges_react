...
export const dataService = {
    ...
    async getVisitChatMessages(visitId: string): Promise<OrderVisitChatMessage[]> {
            ...
            if (readsData) {
                for (const read of readsData as any[]) {
                    ...
                    readsMap[chatId].push({
                        ...
                        userAvatarUrl: this.getPublicImageUrl(read.user?.img_file_path, read.user?.img_file_name, { width: 100, height: 100, resize: 'cover' }),
                        readAt: read.read_at
                    });
                }
            }
        ...
        return data.map((item: any) => {
            ...
            const userAvatarUrl = this.getPublicImageUrl(item.user?.img_file_path, item.user?.img_file_name, { width: 100, height: 100, resize: 'cover' });
    ...
    async sendVisitChatMessage(messageData: Partial<OrderVisitChatMessage> & { activeUserIds?: string[] }): Promise<OrderVisitChatMessage | null> {
        ...
        const userAvatarUrl = this.getPublicImageUrl(data.user?.img_file_path, data.user?.img_file_name, { width: 100, height: 100, resize: 'cover' });
