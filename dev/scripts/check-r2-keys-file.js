...
async function checkR2() {
    ...
    if (data.Contents) {
        data.Contents.forEach(obj => {
            log += `KEY: ${obj.Key} | HAS_SLASH: ${obj.Key.endsWith('/')} | SIZE: ${obj.Size}\n`;
        });
    } else {
        log = "No contents found.";
    }
    fs.writeFileSync('r2_keys_debug.txt', log);
    console.log("Log saved to r2_keys_debug.txt");
}
checkR2();
