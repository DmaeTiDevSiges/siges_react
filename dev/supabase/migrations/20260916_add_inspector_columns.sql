-- Migration: Add inspector columns to assets_loans
-- Date: 2026-09-16
-- Two different inspectors: one for delivery, one for return
-- Both are the logged-in user at the time of the action

-- 1. Add inspector columns
ALTER TABLE assets_loans
    ADD COLUMN IF NOT EXISTS inspector_delivery_user_id BIGINT REFERENCES users(id),
    ADD COLUMN IF NOT EXISTS inspector_return_user_id BIGINT REFERENCES users(id);

-- 2. Recreate view to include inspector info
