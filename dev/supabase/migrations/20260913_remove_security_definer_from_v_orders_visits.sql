-- ============================================================
-- Migration: Remove SECURITY DEFINER from v_orders_visits
-- Date: 2026-09-13
-- Fix: Recriar view sem SECURITY DEFINER para que RLS e
--       permissões do querying user sejam respeitados
-- ============================================================

-- 1. Drop dependent views first
DROP VIEW IF EXISTS public.v_orders_visits_assets CASCADE;

-- 2. Drop the view itself
DROP VIEW IF EXISTS public.v_orders_visits;
...
FROM orders_visits
    ...
    LEFT JOIN users users_costs_approved ON orders_visits.ov_costs_approved_user_id = users_costs_approved.id
    LEFT JOIN users users_costs_rejected ON orders_visits.ov_costs_rejected_user_id = users_costs_rejected.id;

-- 4. Recreate v_orders_visits_assets (was dropped by CASCADE)
