...
CREATE TABLE public.orders_visits (
  id bigint generated by default as identity not null,
  ov_mask character varying null,
  visit_counter integer null,
  o_id bigint null,
  ov_approved_user_id bigint null,
  ov_approved_at timestamp without time zone null,
  ov_disapproved_user_id bigint null,
  ov_disapproved_at timestamp without time zone null,
  ov_started_at timestamp without time zone null,
  ov_ended_at timestamp without time zone null,
  ov_processing_id bigint null default '1'::bigint,
  ov_status_id bigint null default '1'::bigint,
  ov_reported_at timestamp without time zone null,
  ov_reported_user_id bigint null,
  ov_team_leader_id bigint null,
  ov_assets_amount integer null default 0,
  ov_assets_approved_filed_amount integer null default 0,
  ov_assets_disapproved_amount integer null default 0,
  ov_assets_reported_amount integer null default 0,
  ov_materials_value numeric null default '0'::numeric,
  ov_services_value numeric null default '0'::numeric,
  ov_vehicles_value numeric null default '0'::numeric,
  ov_created_user_id bigint null,
  ov_created_at timestamp without time zone null,
  ov_updated_user_id bigint null,
  ov_updated_at timestamp without time zone null,
  ov_deleted_user_id bigint null,
  is_deleted boolean null default false,
  ov_duration_hours double precision null default '0'::double precision,
  ov_disapproved_comments text null,
  ov_assets_draft_amount integer null default 0,
  is_canceled boolean null default false,
  ov_comments text null,
  o_cancel_reason_id bigint null,
  ov_created_latitude double precision null,
  ov_created_longitude double precision null,
  ov_total_value numeric null default '0'::numeric,
  ov_is_filed boolean null default false,
  ov_team_amount integer null default 1,
  ov_assets_revised_amount integer null default 0,
  ov_revised_user_id bigint null,
  ov_revised_at timestamp without time zone null,
  ov_team_names_short character varying null,
  version_mode character varying null default 'live'::character varying,
  ov_rpt_file_path character varying null,
  ov_rpt_file_name character varying null,
  ov_o_suspended_reason_id bigint null,
  ov_o_status_id bigint null default '5'::bigint,
  ov_o_progress numeric null default '0'::numeric,
  ov_img_file_path text null,
  ov_img_file_name text null,
  ov_pdf_file_path text null,
  ov_pdf_file_name text null,
  x_created_user_logon text null,
  x_created_user_id bigint null,
  x_interv_id bigint null,
  is_extra boolean null default false,
  ov_assets_approved_no_filed_amount integer null default 0,
  ov_approved_filed_user_id bigint null,
  ov_approved_filed_at timestamp without time zone null,
  finger_print character varying null,
  ov_payment_at timestamp without time zone null,
  ov_payment_invoices text null,
  ov_started_year smallint null,
  ov_started_month smallint null,
  ov_invoices character varying null,
  ov_assets_approved_amount bigint null,
  created_user_id bigint null,
  created_at timestamp without time zone null,
  updated_user_id bigint null,
  updated_at timestamp without time zone null,
  ov_signature_leader_path text null,
  ov_signature_leader_name character varying null,
  ov_signature_leader_at timestamp without time zone null,
  ov_signature_requester_path text null,
  ov_signature_requester_name character varying null,
  ov_signature_requester_at timestamp without time zone null,
  constraint orders_visits_pkey primary key (id)
) TABLESPACE pg_default;

-- Indexes
create index IF not exists orders_visits_finger_print_idx on public.orders_visits using btree (finger_print) TABLESPACE pg_default;

create index IF not exists idx_orders_visits_o_id on public.orders_visits using btree (o_id) TABLESPACE pg_default;

create index IF not exists idx_ov_ended_at on public.orders_visits using btree (ov_ended_at) TABLESPACE pg_default;

create index IF not exists idx_orders_visits_processing on public.orders_visits using btree (ov_processing_id, ov_status_id) TABLESPACE pg_default;

create index IF not exists idx_orders_visits_started_at on public.orders_visits using btree (ov_started_at desc) TABLESPACE pg_default;

create index IF not exists idx_orders_visits_status on public.orders_visits using btree (ov_status_id) TABLESPACE pg_default;
...
or
update OF ov_started_at on orders_visits for EACH row
execute FUNCTION fc_set_ov_started_date_parts ();

create trigger tgr_durations_hours_decimals BEFORE INSERT
or
update on orders_visits for EACH row
execute FUNCTION fc_durations_hours_decimals ();

create trigger trg_orders_visits_update_total_value
after
update OF ov_materials_value,
ov_services_value,
ov_vehicles_value on orders_visits for EACH row
execute FUNCTION fc_orders_visits_update_total_value ();

create trigger trg_orders_visits_assets_update_services_value
after
update OF ov_services_value,
ov_assets_amount on orders_visits for EACH row
execute FUNCTION fc_orders_visits_assets_update_services_value ();

create trigger trg_orders_visits_assets_update_vehicles_value
after
update OF ov_vehicles_value,
ov_assets_amount on orders_visits for EACH row
execute FUNCTION fc_orders_visits_assets_update_vehicles_value ();
...
-- =============================================================================
-- Section: Custom - Chat de Visitas (Added 2026-05-28)
-- =============================================================================

-- Tabela de Mensagens do Chat de Visitas
CREATE TABLE IF NOT EXISTS public.orders_visits_chat (
    id BIGSERIAL PRIMARY KEY,
    ov_id BIGINT NOT NULL REFERENCES public.orders_visits(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_ovc_ov_id ON public.orders_visits_chat(ov_id);
CREATE INDEX IF NOT EXISTS idx_ovc_user_id ON public.orders_visits_chat(user_id);

ALTER TABLE public.orders_visits_chat ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Enable all for authenticated" ON public.orders_visits_chat
    FOR ALL TO authenticated USING (true) WITH CHECK (true);

-- Tabela de Participantes do Chat de Visitas
CREATE TABLE IF NOT EXISTS public.orders_visits_chat_participants (
    id BIGSERIAL PRIMARY KEY,
    ov_id BIGINT NOT NULL REFERENCES public.orders_visits(id) ON DELETE CASCADE,
    user_id BIGINT NOT NULL REFERENCES public.users(id),
    created_at TIMESTAMPTZ DEFAULT NOW(),
    UNIQUE(ov_id, user_id)
);

CREATE INDEX IF NOT EXISTS idx_ovcp_ov_id ON public.orders_visits_chat_participants(ov_id);

ALTER TABLE public.orders_visits_chat_participants ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Enable all for authenticated" ON public.orders_visits_chat_participants
    FOR ALL TO authenticated USING (true) WITH CHECK (true);

-- Tabela de Leituras do Chat de Visitas (Read Receipts)
CREATE TABLE IF NOT EXISTS public.orders_visits_chat_reads (
    id BIGSERIAL PRIMARY KEY,
    chat_id BIGINT NOT NULL REFERENCES public.orders_visits_chat(id) ON DELETE CASCADE,
    user_id BIGINT NOT NULL REFERENCES public.users(id),
    read_at TIMESTAMPTZ DEFAULT NOW(),
    UNIQUE(chat_id, user_id)
);

CREATE INDEX IF NOT EXISTS idx_ovcr_chat_id ON public.orders_visits_chat_reads(chat_id);
CREATE INDEX IF NOT EXISTS idx_ovcr_user_id ON public.orders_visits_chat_reads(user_id);

ALTER TABLE public.orders_visits_chat_reads ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Enable all for authenticated" ON public.orders_visits_chat_reads
    FOR ALL TO authenticated USING (true) WITH CHECK (true);

-- Inclusão na Publicação Realtime do Supabase
...
-- =============================================================================
-- Tabelas de avisos do app (App Notices)
-- =============================================================================

CREATE TABLE IF NOT EXISTS public.cfg_app_notices_categories (
    id SERIAL PRIMARY KEY,
...
INSERT INTO public.cfg_app_notices_categories (code, label, color, icon, order_index) VALUES
...
CREATE POLICY "Categories: view" ON public.cfg_app_notices_categories
    FOR SELECT TO authenticated USING (TRUE);

CREATE TABLE IF NOT EXISTS public.cfg_app_notices_severities (
    id SERIAL PRIMARY KEY,
    code TEXT NOT NULL UNIQUE,
    label TEXT NOT NULL,
    color TEXT NOT NULL DEFAULT '#6B7280',
...
INSERT INTO public.cfg_app_notices_severities (code, label, color, icon, order_index) VALUES
...
CREATE POLICY "Severities: view" ON public.cfg_app_notices_severities
    FOR SELECT TO authenticated USING (TRUE);

CREATE TABLE IF NOT EXISTS public.cfg_app_notices (
    id BIGSERIAL PRIMARY KEY,
    title TEXT NOT NULL,
    message TEXT NOT NULL,
    category_id INTEGER NOT NULL REFERENCES public.cfg_app_notices_categories(id) ON DELETE RESTRICT,
    severity_id INTEGER NOT NULL REFERENCES public.cfg_app_notices_severities(id) ON DELETE RESTRICT,
...
CREATE INDEX IF NOT EXISTS idx_cfg_app_notices_active ON public.cfg_app_notices(is_active, start_date, end_date);
CREATE INDEX IF NOT EXISTS idx_cfg_app_notices_category ON public.cfg_app_notices(category_id);
CREATE INDEX IF NOT EXISTS idx_cfg_app_notices_severity ON public.cfg_app_notices(severity_id);
CREATE INDEX IF NOT EXISTS idx_cfg_app_notices_created_at ON public.cfg_app_notices(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_cfg_app_notices_date_range ON public.cfg_app_notices(start_date, end_date);
...
CREATE OR REPLACE VIEW public.v_app_notices AS
SELECT
    n.*,
...
DROP TRIGGER IF EXISTS trigger_update_cfg_app_notices_updated_at ON public.cfg_app_notices;
...
-- =============================================================================
-- Section: Gamification System (2026-08-07)
-- =============================================================================

-- View: Score individual por visita encerrada
...
-- Tabela: Snapshot mensal por líder
...
-- Tabela: Histórico detalhado (uma linha por visita avaliada)
...
-- View: Ranking de líderes por departamento e período
...
-- Função: Recalcular scores de um líder para um mês
...
-- Função: Recalcular todos os líderes de um departamento/mês
...
-- Função: Recalcular TODOS os departamentos de um mês
