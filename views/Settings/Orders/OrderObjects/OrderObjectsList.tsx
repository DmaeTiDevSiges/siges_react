...
export const OrderObjectsList: React.FC<OrderObjectsListProps> = ({ onSelect, onAdd }) => {
    ...
    const [objects, setObjects] = useState<OrderObject[]>(listCache?.objects || []);
    ...
    const stateRef = useRef({ objects, visibleCount, search, statusFilter });
    useEffect(() => {
        stateRef.current = { objects, visibleCount, search, statusFilter };
    }, [objects, visibleCount, search, statusFilter]);
    ...
    useEffect(() => {
        return () => {
            if (stateRef.current.objects.length > 0) {
                listCache = {
    ...
    useEffect(() => {
        const fetchData = async () => {
            if (!listCache && objects.length === 0) setLoading(true);

            try {
                setError(null);
                const data = await dataService.getOrderObjects(statusFilter, debouncedSearch);
                setObjects(data);
            } catch (error) {
                console.error('Failed to load objects', error);
                setError('Erro ao carregar objetos. Verifique sua conexão.');
            } finally {
                setLoading(false);
            }
        };
    ...
    const hasMore = visibleCount < objects.length;

    const handleLoadMore = () => {
        setVisibleCount(prev => Math.min(prev + PAGE_SIZE, objects.length));
    };

    // We use the LoadMore component which handles IntersectionObserver internally

    if (loading && !listCache) return <div className="p-8 text-center text-slate-500">Carregando objetos...</div>;
...

                {
                    visibleObjects.length === 0 && (
                        <div className="text-center py-12 text-slate-500">
                            {search ? 'Nenhum objeto encontrado' : 'Nenhum objeto cadastrado'}
                        </div>
                    )
                }
