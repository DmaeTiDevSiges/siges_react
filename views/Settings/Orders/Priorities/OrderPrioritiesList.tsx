...
export const PrioritiesList: React.FC<PrioritiesListProps> = ({ onSelect, onAdd }) => {
    ...
    const [priorities, setPriorities] = useState<Priority[]>(listCache?.priorities || []);
    ...
    const stateRef = useRef({ priorities, visibleCount, search, statusFilter });
    useEffect(() => {
        stateRef.current = { priorities, visibleCount, search, statusFilter };
    }, [priorities, visibleCount, search, statusFilter]);
    ...
    useEffect(() => {
        return () => {
            if (stateRef.current.priorities.length > 0) {
                listCache = {
                    priorities: stateRef.current.priorities,
    ...
    useEffect(() => {
        const fetchData = async () => {
            if (!listCache && priorities.length === 0) setLoading(true);

            try {
                setError(null);
                const data = await dataService.getPriorities(statusFilter, debouncedSearch);
                setPriorities(data);
            } catch (error) {
    ...
    const visiblePriorities = priorities.slice(0, visibleCount);
    const hasMore = visibleCount < priorities.length;

    const handleLoadMore = () => {
        setVisibleCount(prev => Math.min(prev + PAGE_SIZE, priorities.length));
    };

    // We use the LoadMore component which handles IntersectionObserver internally

    if (loading && !listCache) return <div className="p-8 text-center text-slate-500">Carregando prioridades...</div>;
        ...
        <div className="flex flex-col h-full bg-background-light dark:bg-background-dark">
            ...
            >
...

                {
                    visiblePriorities.length === 0 && (
                        <div className="text-center py-12 text-slate-500">
                            {search ? 'Nenhuma prioridade encontrada' : 'Nenhuma prioridade cadastrada'}
                        </div>
                    )
                }

                {
                    visiblePriorities.length > 0 && (
                        <LoadMore
                            current={visiblePriorities.length}
