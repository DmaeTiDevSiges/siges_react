...
export const OrderPlansList: React.FC<OrderPlansListProps> = ({ onSelect, onAdd }) => {
    ...
    const [plans, setPlans] = useState<OrderPlan[]>(listCache?.plans || []);
    ...
    const stateRef = useRef({ plans, visibleCount, search, statusFilter });
    useEffect(() => {
        stateRef.current = { plans, visibleCount, search, statusFilter };
    }, [plans, visibleCount, search, statusFilter]);
    ...
    useEffect(() => {
        return () => {
            if (stateRef.current.plans.length > 0) {
                listCache = {
                    plans: stateRef.current.plans,
    ...
    useEffect(() => {
        const fetchData = async () => {
            if (!listCache && plans.length === 0) setLoading(true);

            try {
                setError(null);
                const data = await dataService.getOrderPlans(statusFilter, debouncedSearch);
                setPlans(data);
            } catch (error) {
    ...
    const handleLoadMore = () => {
        setVisibleCount(prev => Math.min(prev + PAGE_SIZE, plans.length));
    };
