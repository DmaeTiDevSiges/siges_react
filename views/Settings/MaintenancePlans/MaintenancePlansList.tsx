...
interface MaintenancePlansListProps {
    onSelect: (item: MaintenancePlan) => void;
    onAdd: () => void;
    onBack: () => void;
}
...
export const MaintenancePlansList: React.FC<MaintenancePlansListProps> = ({ onSelect, onAdd, onBack }) => {
    ...
    const [items, setItems] = useState<MaintenancePlan[]>(listCache?.items || []);
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                setError(null);
                const data = await dataService.getMaintenancePlans();
                ...
                if (debouncedSearch) {
                    ...
                    filtered = filtered.filter(p => p.description.toLowerCase().includes(l) || p.code?.toLowerCase().includes(l));
                }
    ...
    if (loading && !listCache) return <div className="p-8 text-center text-slate-500">Carregando planos...</div>;
    ...
    return (
        <div className="flex flex-col h-full bg-background-light dark:bg-background-dark">
                ...
                <div className="flex items-center gap-3">
                    <div className="flex-1">
                        <SearchInput
                            ...
                            placeholder="Buscar planos de manutenção..."
                        />
                    </div>
                    <IconButton
                        icon="add"
            ...
            <div
                ...
                className="flex-1 overflow-y-auto p-4 pb-32 space-y-3"
            >
                {visibleItems.map((item) => (
                    <div
                        key={item.id}
                        onClick={() => onSelect(item)}
                        className="bg-white dark:bg-slate-900/40 rounded-[24px] p-5 border border-slate-200 dark:border-slate-800 hover:border-primary/50 hover:shadow-lg hover:shadow-primary/5 transition-all cursor-pointer flex items-center justify-between gap-4 group"
                    >
                        <div className="flex items-center flex-1 min-w-0 gap-4">
                            <div className="w-12 h-12 rounded-2xl bg-slate-50 dark:bg-slate-800 flex items-center justify-center text-primary group-hover:bg-primary group-hover:text-white transition-all shrink-0">
