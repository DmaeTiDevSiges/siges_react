...
export const AssetStatusesList: React.FC<AssetStatusesListProps> = ({ onSelect, onAdd }) => {
    ...
    const [items, setItems] = useState<AssetStatus[]>(listCache?.items || []);
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                setError(null);
                const data = await dataService.getAssetStatuses(statusFilter, debouncedSearch);
                setItems(data);
            } catch (error) {
    ...
    if (loading && !listCache) return <div className="p-8 text-center text-slate-500">Carregando situações...</div>;
                ...
                {visibleItems.map((item) => (
                    ...
                    >
                        <div className="flex flex-col flex-1 min-w-0">
                            ...
                            <div className="text-xs text-slate-500 mt-1 pl-6">{item.code}</div>
                        </div>
...

                {
                    visibleItems.length === 0 && (
                        <div className="text-center py-12 text-slate-500">
                            {search ? 'Nenhuma situação encontrada' : 'Nenhuma situação cadastrada'}
                        </div>
                    )
                }
