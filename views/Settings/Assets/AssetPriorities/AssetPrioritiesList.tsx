...
export const AssetPrioritiesList: React.FC<AssetPrioritiesListProps> = ({ onSelect, onAdd }) => {
    ...
    const [items, setItems] = useState<AssetPriority[]>(listCache?.items || []);
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                setError(null);
                const data = await dataService.getAssetPriorities(statusFilter, debouncedSearch);
                setItems(data);
            } catch (error) {
                console.error('Failed to load asset priorities', error);
