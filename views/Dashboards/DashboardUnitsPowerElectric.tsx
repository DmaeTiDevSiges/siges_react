...
const FilterBarSection = React.memo(({ 
    ...
    return (
        <div className="flex items-center gap-2 overflow-x-auto no-scrollbar w-full pb-1">
            ...
            <FilterSelect label="SUB-TIPO UNIDADE" value={advancedFilters.unitTypeId || []} onClick={() => openSelectionModal('unitTypeId', 'SUB-TIPO UNIDADE', filterOptions.unitSubTypes)} onClear={() => setAdvancedFilters(prev => ({ ...prev, unitTypeId: [] }))} hidden={!advancedFilters.unitTypeParentId || (Array.isArray(advancedFilters.unitTypeParentId) && advancedFilters.unitTypeParentId.length === 0)} />
            ...
            <FilterSelect label="SUB-TIPO OS" value={advancedFilters.orderTypeSubId || []} onClick={() => openSelectionModal('orderTypeSubId', 'SUB-TIPO OS', filterOptions.orderSubTypes)} onClear={() => setAdvancedFilters(prev => ({ ...prev, orderTypeSubId: [] }))} hidden={!advancedFilters.orderTypeId || (Array.isArray(advancedFilters.orderTypeId) && advancedFilters.orderTypeId.length === 0)} />
        ...
        >
            <div className="flex justify-between items-start mb-2">
                ...
                {totalValue !== undefined && (
                    <span className={`text-xl font-black transition-all duration-300 ${active ? 'text-primary' : 'text-slate-900 dark:text-white'}`} style={color && !active ? { color } : undefined}>
