...
def run_tests(cmd: list, cwd: Path) -> dict:
    ...
    try:
        ...
        # Try to parse test counts from output (check both stdout and stderr)
        output = (proc.stdout or "") + (proc.stderr or "")
        
        # Jest/Vitest pattern: "Tests: X passed, Y failed, Z total"
        if "passed" in output.lower() and "total" in output.lower():
            match = re.search(r'Tests:\s+(\d+)\s+passed,\s+(\d+)\s+total', output, re.IGNORECASE)
            if match:
                result["tests_passed"] = int(match.group(1))
                result["tests_run"] = int(match.group(2))
                result["tests_failed"] = result["tests_run"] - result["tests_passed"]
            else:
                # Alternative pattern: "Tests: X passed, Y total"
